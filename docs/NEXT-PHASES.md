...
1. **Zero-row fallback bug** — ✅ **DONE (Track 1.3 U3, 2026-08-21)**. All 3 catalog list APIs (universities, programs, scholarships) now only fall back to static when no filters/search are applied AND the DB is genuinely empty. A filter that legitimately matches nothing returns `[]` as expected. Diff: `src/app/api/{universities,programs,scholarships}/route.ts`.
...
3. **`/api/seed` has no auth guard** — ✅ **DONE (Track 1.3 U3, 2026-08-21)**. Added `requireAdmin(request)` to the POST handler, consistent with every other mutation route. Diff: `src/app/api/seed/route.ts`.

4. **Scholarship form↔schema mismatch** — ✅ **DONE (Track 1.3 U3, 2026-08-21)**. Form now passes `applicationMethod` + `applicationMethodCn` as plain strings (they were being split by newlines + filtered into an array, but the form fields are single-line `<input>`s, the validator is `z.string()`, and the DB column is `text`). Submits no longer 400. **Related drift still open**: the schema accepts `typeCn, degreeLevelsCn, eligibleRegionsCn, durationCn, benefits, benefitsCn, officialLink` but `mapScholarshipToDb` drops them and the DDL has no such columns — trim the schema in a follow-up or add the columns. Diff: `src/app/admin/scholarships/new/page.tsx`.
