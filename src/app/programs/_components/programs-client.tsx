...
import { Search, Filter, GraduationCap, Globe, Clock, Banknote, ArrowRight, Award, X } from 'lucide-react';
...
export default function ProgramsClient({ initialPrograms, initialUniversities }: ProgramsClientProps) {
  const { t, locale } = useI18n();
  const searchParams = useSearchParams();
  const [currentPage, setCurrentPage] = useState(1);
  // Seed state from the server-fetched lists so the first paint already
