...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('[partner/promotions GET] supabase error:', error);
    ...
    const result = promotions.map((p) => ({
      ...p,
      isCountryEligible:
        !partnerCountry || isPromotionEligibleForCountry(p, partnerCountry),
    }));

    return NextResponse.json({ promotions: result });
  } catch (error) {
    console.error('[partner/promotions GET] error:', error);
