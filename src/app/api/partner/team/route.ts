 ...
 *
 * Phase 84: invite delivery now goes through sendTemplatedEmail
 * (loads the notification.partner_invite template, renders with
 * {{var}} substitution, ships via Resend). The inline HTML+text
 * body was dropped — the email_templates row is the single
 * source of truth for invite copy.
 */
 ...
 * - Sends the invite via sendTemplatedEmail (notification.partner_invite)
 */
export async function POST(request: NextRequest) {
  ...
  // 5. Send the invite email via the centralized template pipeline.
  ...
  if (!sent) {
    // Don't roll back the team_members row on send failure — the
    // owner can resend via the partner team UI. Just surface a
    // warning so the caller knows the email didn't go out.
    return NextResponse.json({
      member,
      inviteSentTo: email,
      isNewUser,
      warning: 'Invite email failed to send. Use Resend to try again.',
    });
  }
