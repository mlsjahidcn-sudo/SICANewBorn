...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  let universities: SitemapEntry[] = staticUniversities.map((u) => ({
    slug: u.slug,
  }));
  ...
  const universityUrls: MetadataRoute.Sitemap = universities.map((entry) => ({
  ...
  const scholarshipUrls: MetadataRoute.Sitemap = scholarships.map((entry) => ({
  ...
  return [...staticPages, ...universityUrls, ...scholarshipUrls];
}
